...
class HttpResponse:
    ...
    def json(cls, payload: Any, status_code: int = 200) -> "HttpResponse":
        return cls(status_code=status_code, body=json.dumps(payload).encode("utf-8"), content_type="application/json")

    @classmethod
    def text(cls, text: str, status_code: int = 200, content_type: str = "text/plain; charset=utf-8") -> "HttpResponse":
        return cls(status_code=status_code, body=text.encode("utf-8"), content_type=content_type)
...
class MiniApp:
    ...
    def __call__(self, environ: dict[str, Any], start_response: Callable[..., Any]) -> list[bytes]:
        ...
        try:
            ...
            return HttpResponse.json({"error": "Not found"}, status_code=404).to_wsgi(start_response)
        except Exception as error:  # pragma: no cover - emergency guard
            return HttpResponse.json({"error": str(error)}, status_code=500).to_wsgi(start_response)
    ...
    def _build_request(self, environ: dict[str, Any]) -> dict[str, Any]:
        ...
        request: dict[str, Any] = {
            "method": environ.get("REQUEST_METHOD", "GET").upper(),
            "path": environ.get("PATH_INFO", "/"),
            "query": {key: values[-1] for key, values in parse_qs(environ.get("QUERY_STRING", "")).items()},
            "body": raw_body,
            "json": None,
            "files": {},
        }
    ...
    def _serve_static(self, path: str) -> HttpResponse:
        if path == "/":
            ...
            if self.config.public_dir.resolve() not in target.parents and target != self.config.public_dir.resolve():
        ...
        return HttpResponse(status_code=200, body=target.read_bytes(), content_type=mime_type)
...
def create_app(root_dir: Path | str) -> MiniApp:
    ...
    def api_events(request: dict[str, Any], _match: re.Match[str] | None) -> HttpResponse:
        payload = load_json_file(config.public_dir / "events.json", {"items": [], "includeClasses": False})
        include_classes = request["query"].get("includeClasses")
        if include_classes == "false":
            payload["items"] = [item for item in payload.get("items", []) if not item.get("isClass")]
            payload["total"] = len(payload["items"])
            payload["includeClasses"] = False
        return HttpResponse.json(payload)

    @app.route("GET", r"/api/slides")
    def api_slides(_request: dict[str, Any], _match: re.Match[str] | None) -> HttpResponse:
        payload = load_json_file(config.public_dir / "slides.json", {"slides": [], "total": 0})
        return HttpResponse.json(payload)

    @app.route("GET", r"/api/videos")
    def api_videos(_request: dict[str, Any], _match: re.Match[str] | None) -> HttpResponse:
        return HttpResponse.json({"items": database.list_videos()})

    @app.route("GET", r"/api/video-associations")
    def api_video_associations(_request: dict[str, Any], _match: re.Match[str] | None) -> HttpResponse:
        return HttpResponse.json({"items": database.list_event_video_map()})

    @app.route("POST", r"/api/videos")
    def api_upload_video(request: dict[str, Any], _match: re.Match[str] | None) -> HttpResponse:
        ...
        stored_name = f"{Path(filename).stem}-{file_hash}{Path(filename).suffix.lower()}"
        ...
        return HttpResponse.json(
            {"video": {"id": video_id, "src": storage_path, "title": Path(filename).stem}},
            status_code=201,
        )

    @app.route("PUT", r"/api/video-associations/(?P<event_id>[^/]+)")
    def api_set_association(request: dict[str, Any], match: re.Match[str] | None) -> HttpResponse:
        ...
        return HttpResponse.json({"eventId": event_id, "videoId": video_id})

    @app.route("DELETE", r"/api/video-associations/(?P<event_id>[^/]+)")
    def api_clear_association(_request: dict[str, Any], match: re.Match[str] | None) -> HttpResponse:
        event_id = match.group("event_id") if match else ""
        database.clear_event_video(event_id)
        return HttpResponse.json({"eventId": event_id, "cleared": True})
